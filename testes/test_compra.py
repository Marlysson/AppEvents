...
class TestCompraInscricao(unittest.TestCase):
	...
	def setUp(self):
		...
		self.cupom_valido1 = Cupom("LOTE_1",0.5,horarios_cupons.get("valido1"))
		self.cupom_valido1.regra = DescontoGeral()

		self.cupom_valido2 = Cupom("TUTORIAIS_30",0.3,horarios_cupons.get("valido2"))
		self.cupom_valido2.regra = DescontoTutorial()
		
		self.cupom_invalido1 = Cupom("WORKSHOP_10",0.1,horarios_cupons.get("invalido1"))
		self.cupom_invalido1.regra = DescontoWorkshop()
		
		self.cupom_invalido2 = Cupom("HACKATHON_50",0.5,horarios_cupons.get("invalido2"))
		self.cupom_invalido2.regra = DescontoHackathon()
		...
		self.evento.adicionar_cupom(self.cupom_valido1)
		self.evento.adicionar_cupom(self.cupom_valido2)
		self.evento.adicionar_cupom(self.cupom_invalido1)
	...
	def test_nao_deve_aplicar_descontos_de_cupons_nao_ativos(self):
		...
		inscricao.adicionar_atividade(self.tutorial)
		inscricao.adicionar_atividade(self.workshop1)
		inscricao.adicionar_atividade(self.workshop2)
		inscricao.adicionar_atividade(self.hackathon)

		compra = Compra(inscricao)

		with self.assertRaises(CupomExpirado):
			compra.aplicar_cupom(self.cupom_invalido1)
	...
	def test_deve_gerar_excecao_quando_aplicar_um_cupom_nao_cadastrado_no_evento(self):
		...
		with self.assertRaises(CupomNaoEncontradoNoEvento):
			compra.aplicar_cupom(self.cupom_invalido2)
